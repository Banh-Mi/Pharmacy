...
public class ChitietHoadonBanJframe extends javax.swing.JFrame {
    ...
    public ChitietHoadonBanJframe(NhanVien nv, KhachHang kh, ArrayList<Thuoc> dsThuocTrongThem, String tienKhach, String tienThoi, boolean tf_XuatHoaDon) {
        initComponents();
        tblCtHD.getColumnModel().getColumn(2).setPreferredWidth(50);
        setLocationRelativeTo(null);
        ...
        if (tf_XuatHoaDon) {
            btnDong.setText("Xuất Hóa Đơn");
        } else {
            btnDong.setText("Đóng");
        }
//        this.thuoc = thuoc;
        docVaoChiTiet2(nv, kh);
        docDataCtBanVaoTable2(dsThuocTrongThem);
        tinhTongTien2();
        txtTongVAT.setText(tienKhach);
        txtTongTien.setText(tienThoi);
    ...
    private void initComponents() {
        ...
        if (tblCtHD.getColumnModel().getColumnCount() > 0) {
            ...
            tblCtHD.getColumnModel().getColumn(0).setPreferredWidth(5);
            tblCtHD.getColumnModel().getColumn(1).setResizable(false);
            tblCtHD.getColumnModel().getColumn(1).setPreferredWidth(50);
            tblCtHD.getColumnModel().getColumn(2).setResizable(false);
            tblCtHD.getColumnModel().getColumn(2).setPreferredWidth(150);
...
//		jLabel5.setText("Bằng chữ:");
    ...
    private void btnDongActionPerformed(java.awt.event.ActionEvent evt) {
        ...
        if (btnDong.getText().equals("Xuất Hóa Đơn")) {
            try {
                captureFrame(this);
    ...
    private void captureFrame(JFrame frame) throws Exception {
        ...
        System.out.println("getShape" + frame.getShape());
        frame.setBounds(frame.getBounds().x, frame.getBounds().y, 813, 720);
        BufferedImage image = new Robot().createScreenCapture(frame.getBounds());
        ImageIO.write(image, "png", new File("src\\xuatfile\\hoadon" + txtMaHd.getText() + ".png"));
    }
...
//        daohdb = new dao.DAO_HoaDonBan();
        HoaDonBan hdb = daohdb.getChiTietHoaDonBan(maHd);
    ...
    private void docVaoChiTiet2(NhanVien nv, KhachHang kh) {
        Date date = new Date();
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
        String strDate = sdf.format(date);
        String maHDCuaHang = daohdb.taoMaNgauNhien();
    ...
    private void docDataCtBanVaoTable() {
        ...
        ArrayList<CT_HoaDonBan> dscthd = daocthd.getChiTietHoaDon(maHd);
        DefaultTableModel md = (DefaultTableModel) tblCtHD.getModel();
        md.getDataVector().removeAllElements();
        dscthd.forEach(t -> {
            i++;
            md.addRow(new Object[]{
                i, t.getThuoc().getTenThuoc(), t.getGiaBan(), t.getThuoc().getThanhPhan(), t.getVat() * 100, t.getSoLuong(),
    ...
    private void docDataCtBanVaoTable2(ArrayList<Thuoc> dscthd) {
//        daocthd = new DAO_CT_HoaDonBan();
//        ArrayList<CT_HoaDonBan> dscthd = daocthd.getChiTietHoaDon(maHd);
        DefaultTableModel md = (DefaultTableModel) tblCtHD.getModel();
        md.getDataVector().removeAllElements();
        dscthd.forEach(t -> {
            i++;
            md.addRow(new Object[]{
                i, t.getTenThuoc(), t.getGiaBan(), t.getThanhPhan(), t.getThueVAT() * 100, t.getSoLuongBanDau(),
                Math.round(t.getGiaBan() * (1 + t.getThueVAT()) * t.getSoLuongBanDau())
            });
        });
    ...
    private void tinhTongTienHD() {
        ...
        String fmtien = String.format("%,.0f", tongTien);
        txtTongHD.setText(fmtien + " VNĐ");
    }
    ...
    private void tinhTongTienVat() {
        ...
        for (int j = 0; j < count; j++) {
            tongTien += Double.parseDouble(tblCtHD.getValueAt(j, 2).toString())
                    * Double.parseDouble(tblCtHD.getValueAt(j, 5).toString())
                    * Double.parseDouble(tblCtHD.getValueAt(j, 4).toString()) * 0.01;
        }
        String fmtien = String.format("%,.0f", tongTien);
        txtTongVAT.setText(fmtien + " VNĐ");
    }
    ...
    private void tinhTongTien() {
        ...
        String fmtien = String.format("%,.0f", tongTien);
        txtTongTien.setText(fmtien + " VNĐ");
    }
